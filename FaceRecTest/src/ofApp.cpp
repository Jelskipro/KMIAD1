...
void ofApp::calcFaceSprites() {
	...
	for (int i = 0; i<rec.numPeople(); i++) {
		...
		ofPixels pixels = rec.getPersonPixels(i);
...
void ofApp::update() {
	...
	if (vidGrabber.isFrameNew()) {
		...
		//                       }
		//		}
		img.setFromPixels(pixels.getData(), camWidth, camHeight, OF_IMAGE_COLOR);
		test_image.setFromPixels(pixels.getData(), camWidth, camHeight, OF_IMAGE_COLOR);
...
void ofApp::draw() {
	...
	for (int i = 0; i < finder.blobs.size(); i++) {
		...
		ofPixels pixels = gray.getPixels();
		for (int x = 0; x<PCA_WIDTH; x++)
			for (int y = 0; y<PCA_HEIGHT; y++)
